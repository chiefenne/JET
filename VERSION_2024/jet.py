...
class Jet():
    ...
    def __init__(self, Reynolds=30000, Prandtl=0.7, Prandtl_turb=0.9, turbulent=True):
        """
    ...
    def set_Reynolds(self, velocity, half_width, viscosity):
        """
        Set the Reynolds number.

        Args:
            velocity (float): The velocity of the fluid.
            half_width (float): The half_width of the jet.
            viscosity (float): The viscosity of the fluid.
        """
        self.Reynolds = velocity * half_width / viscosity

    def set_Prandtl(self, viscosity, thermal_conductivity, specific_heat):
        """
        Set the Prandtl number.

        Args:
            viscosity (float): The viscosity of the fluid.
            thermal_conductivity (float): The thermal conductivity of the fluid.
            specific_heat (float): The specific heat of the fluid.
        """
        self.Prandtl = viscosity * specific_heat / thermal_conductivity
    ...
    def plot(self, steps=[]):
        ...
        for step in steps:
            ...
            text1 = '\n'.join((
                ...
                r'$\xi={:.2g}$'.format(result[1]),
                r'$\eta_m={:.2g}$'.format(self.eta[-1]),
                r'$Reynolds={:d}$'.format(int(self.Reynolds)),
                r'$Prandtl={:.2g}$'.format(self.Prandtl),
                r'$Prandtl_t={:.2g}$'.format(self.Prandtl_turb)))

            text2 = '\n'.join((
                r'$\xi_0={:.2g}$'.format(self.gsi[0]),
                r'$d\xi={:.2g}$'.format(self.dgsi),
                r'$d\eta_0={:.2g}$'.format(self.deta[0]),
                r'$\eta_e={:.2g}$'.format(self.etae),
                r'$stretch\;factor={:.2g}$'.format(self.stretch)))
            ...
            if self.turbulent:
                ...
                ax.set_title('2D Laminar Heated Free Jet', fontsize=16)
...
if __name__ == "__main__":
    ...
    # fluid properties (temperature in C, anything between 0 and 100)
    temperature = 25.0

    # set fluid (can be any of 'air', 'water', 'oil')
    fluid = 'water'

    # geometry (nozzle width in m)
    nozzle_width = 0.1
    ...
    fluid_properties = fp.data
    temperatures = fluid_properties[fluid]['temperatures']
    ...
    thermal_conductivity = fluid_properties[fluid]['thermal_conductivity']
    specific_heat = fluid_properties[fluid]['specific_heat']

    # interpolate fluid properties based on temperature
    density = np.interp(temperature, temperatures, density)
    viscosity = np.interp(temperature, temperatures, viscosity)
    kinematic_viscosity = viscosity / density
    thermal_conductivity = np.interp(temperature, temperatures, thermal_conductivity)
    specific_heat = np.interp(temperature, temperatures, specific_heat)

    jet = Jet()

    # Reynolds number calculation based on the half nozzle width
    jet.set_Reynolds(velocity, nozzle_width / 2, kinematic_viscosity)

    # Prandtl number
    jet.set_Prandtl(viscosity, thermal_conductivity, specific_heat)

    # Turbulence
    jet.turbulent = turbulent
    jet.Prandtl_turb = Prandtl_turb

    jet.plot_folder = plot_folder
    jet.result_folder = result_folder

    # make mesh
    jet.mesh(gsimax=30, dgsi=0.01, etae=25, deta1=0.01, stretch=1.1)
